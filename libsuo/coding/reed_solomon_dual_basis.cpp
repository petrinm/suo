

#include "coding/reed_solomon.hpp"

using namespace suo;

#if 1

const uint8_t suo::ToDualBasis[256] = {
	
};

const uint8_t suo::FromDualBasis[256] = {
	
};

void suo::generateDualBasisLookupTable() { }

#else 

uint8_t suo::ToDualBasis[256];
uint8_t suo::FromDualBasis[256];


void suo::generateDualBasisLookupTable() {

	// Transformation matrix from alpha to dual basis
	const uint8_t tal[] = { 0x8d, 0xef, 0xec, 0x86, 0xfa, 0x99, 0xaf, 0x7b };
	
	/* For each value of input */
	for (unsigned int i = 0;i < 256; i++) {
		unsigned int val = 0;

		/* For each column of matrix */
		for (unsigned int j = 0;j < 8;j++) { 

			/* For each row of matrix */
			for (unsigned int k = 0; k < 8; k++) { 
				if (i & (1 << k))
					val ^= tal[7 - k] & (1 << j);
			}
		}

		ToDualBasis[i] = val;
		FromDualBasis[val] = i;
	}
}
#endif


void suo::convertToDualBasis(ByteVector& data) {
	for (int i = 0; i < data.size(); i++)
		data[i] = FromDualBasis[data[i]];
}

void suo::convertFromDualBasis(ByteVector& data) {
    for (int i = 0; i < data.size(); i++)
		data[i] = ToDualBasis[data[i]];
}

